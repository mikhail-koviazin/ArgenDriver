...
Отдельный счетчик заходов на веб-деплой, включен в проекте `argen-driver` на плане Hobby (50k событий в месяц, 30 дней истории, кастомные события только на Pro). Реализация - `app/services/webAnalytics/`, тот же платформенный сплит, что у телеметрии: на вебе в `<head>` добавляется `/_vercel/insights/script.js`, на нативе пустая заглушка. Пакет `@vercel/analytics` не нужен, скрипт отдает сам Vercel с origin деплоя.

Он намеренно **не** привязан к тумблеру согласия. Firebase отвечает на вопрос «сколько пользователей, давших согласие», а этот счетчик - на вопрос «заходит ли вообще кто-нибудь», и согласие сделало бы второй вопрос таким же неотвечаемым, как первый. Cookies не ставятся, идентификатор не хранится, персональных данных нет, так что соглашаться тут не на что. В `__DEV__` скрипт не подключается: локально этот путь не отдается.

Грабли: catch-all rewrite в `vercel.json` съедал `/_vercel/insights/script.js` и отдавал вместо него `index.html`. Поэтому source правила исключает служебный префикс: `/:path((?!_vercel/).*)`. Если понадобится трогать rewrites, проверять надо именно этот путь, а не главную.
...
### Свой трафик не попадает в отчеты

В GA4 у веб-потока есть правило internal traffic с IP наших нод (Vultr Токио, Осака, Сеул, Сингапур и Masterhost Финляндия), а фильтр `Internal Traffic` переведен в состояние Active. События с `traffic_type=internal` отбрасываются на приеме, задним числом это не работает.

Практическое следствие: проверять телеметрию через свой VPN бесполезно, в отчетах будет ноль. Нужен либо выключенный VPN, либо телефон на мобильном интернете, либо DebugView. У потоков Android и iOS такого правила нет - фильтр по IP бывает только у веба, для приложений `traffic_type` пришлось бы отправлять из кода.
